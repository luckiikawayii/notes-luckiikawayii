...
window.addEventListener('load', function() {
 ...
 // part5
         // 获取所有.partFive元素
         let partFives = document.querySelectorAll('.partFive');

         // 设置选中态样式
         function setActive() {
             // 遍历所有.partFive元素，移出active样式
             partFives.forEach((partFive) => {
                partFive.classList.remove('active');
             })
             // 为当前选中项添加active样式
             this.classList.add('active');
         }
         // 遍历所有.partFive元素，分别为其设置点击事件
         partFives.forEach((partFive) => {
            partFive.addEventListener('click', setActive)});
  //part6
           //点击外部标签播放视频
         function clickPlay() {
